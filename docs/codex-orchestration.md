...
The worker must save a durable report first, then notify the parent. Use `ready_for_review`, `blocked`, or `not_done` as the worker terminal status; only the parent may mark the work accepted.
