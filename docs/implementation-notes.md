...
- Comments are copied from `rawsql-ts` `comments`, `positionedComments`, and select-item `aliasPositionedComments`. CTE comments are defined as comments immediately before the CTE name or comments immediately before the CTE's inner SELECT query. Inline comments after a SELECT item can appear on the next item as a `before` positioned comment, so the adapter treats the next item's `before` comments as comments for the previous output column.
- Output column expressions are formatted from the select item value with `rawsql-ts` `Formatter` and stored as display metadata. This is UI metadata, not a canonical SQL rewrite contract.
- Executable SQL for a CTE is generated with `rawsql-ts` `CTEQueryDecomposer.extractCTE(query, cteName)` and stored as display/copy metadata on the CTE node. The adapter snapshots CTE comments before invoking the decomposer because the decomposer can affect comment visibility on the parsed AST.
