# Gate 1 Real-Host MCP Evidence Index

## Final Outcome

- Human decision: **Option 1 accepted**.
- Product Dogfooding Gate 1: **meets / CLOSED**.
- Accepted real-host boundary: interactive Codex CLI actionable and blocked trials.
- Attempt 5: **partial — non-interactive host approval limitation**.
- Product defect indicated: **no**.
- Safety defect indicated: **no**.

Interactive CLI transcript evidence is human-observed supplementary evidence. It is accepted for this Gate, but its raw transcript is not copied into repository artifacts.
...
The durable Attempt 5 report records two `create_investigation_plan` attempts, both cancelled by the host approval path before results. It also records no evaluated-host shell use, DB connection, SQL execution, invented SQL, or alternate tool. This index preserves the report path and immutable hash; it does not duplicate raw event payloads.

## Interpretation Boundary

The evidence proves the server can start, direct MCP operations can complete, and interactive CLI is accepted as the real-host Product Gate evidence. It does not establish unattended local-MCP approval or Codex App GUI tool exposure. Those are separate environment limitations, not product defects.
